...
RotamerLibrary::RotamerRange RotamerLibrary::get_rotamers_fast(
  ...
  RotamerIterator rot_end = library_[res_index][angle_index].end();
  std::vector<RotamerIterator> selected_rotamers;

  double prob_so_far = 0;
  double xrot = IMP::get_random_double_uniform(0.0,1.0);
  double flip = 0;

  //the issue is that rot_begin is not the same thing that boost_make)iterator_range needs
  //loop rotamers and select randomly based on their probabilities
  while (flip < 1 && rot_end != library_[res_index][angle_index].end()) {
    if (prob_so_far + rot_end->get_probability() <= xrot * probability_thr) {
