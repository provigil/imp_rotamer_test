...
AngleConfiguration angle_configurations[NRes] = {
    ...
    {"ASN",
     ...
     {3, -1, -1}},
    {"ASP", {"N-CA-CB-CG", "OD1", 0, 0}, "HB2,HB3,CG,OD1,OD2", {3, -1, -1}},
    {"CYS", {"N-CA-CB-SG", 0, 0, 0}, "HB2,HB3,SG,HG", {-1, -1, -1}},
    {"GLN",
     {"N-CA-CB-CG", "CD", "OE1", 0},
    ...
    {"PRO",
     ...
     {3, -1, -1}},
    {"SER", {"N-CA-CB-OG", 0, 0, 0}, "HB2,HB3,OG,HG", {-1, -1, -1}},
    {"THR",
     {"N-CA-CB-OG1", 0, 0, 0},
...
double get_psi_angle(IMP::atom::Residue rd) {
  ...
  if (psi_atoms.empty()) return 0;
  return IMP::core::get_dihedral(
      IMP::core::XYZ(psi_atoms[0]), IMP::core::XYZ(psi_atoms[1]),
      IMP::core::XYZ(psi_atoms[2]), IMP::core::XYZ(psi_atoms[3]));
}

double get_phi_angle(IMP::atom::Residue rd) {
  IMP::atom::Atoms phi_atoms = IMP::atom::get_phi_dihedral_atoms(rd);
  // if residue is at the beginning or end of the chain, assume 0
  if (phi_atoms.empty()) return 0;
  return IMP::core::get_dihedral(
      IMP::core::XYZ(phi_atoms[0]), IMP::core::XYZ(phi_atoms[1]),
      IMP::core::XYZ(phi_atoms[2]), IMP::core::XYZ(phi_atoms[3]));
}
}
  ...
  for (int i = 0; i < NRes; ++i) {
    ...
    do {
      ...
      for (size_t k = current_start; k != rot_atoms.size(); ++k) {
        ...
        if (idx >= res_d.rot_atoms.size()) res_d.rot_atoms.resize(idx + 1, 0);
    ...
    } while (true);
    {
      ...
      for (int j = 1; j < 4; ++j)
        if (ac.chi_axis[j]) at_axes_names.push_back(ac.chi_axis[j]);
...
const IMP::algebra::Vector3D &ResidueRotamer::get_coordinates(
...
unsigned ResidueRotamer::get_number_of_cases(const IMP::atom::AtomType &at)
    const {
  unsigned idx = at.get_index();
  if (idx >= residue_coordinates_.size()) return 0;
...
IMP::algebra::Vector3D &ResidueRotamer::get_coordinates(
  ...
  unsigned at_idx = at.get_index();
  IMP_USAGE_CHECK(at_idx < residue_coordinates_.size() &&
                      !residue_coordinates_[at_idx].empty(),
                  "no atom of this type present");
...
double ResidueRotamer::get_probability(unsigned index) const {
  ...
  //std::cout << "hello world getprob" << std::endl
}
...
void ResidueRotamer::create_bounding_boxes(ResidueRotamer::Box3D &bb_box,
                                           ResidueRotamer::Box3D &sc_box,
                                           ResidueRotamer::Boxes3D &rot_boxes) {
  bb_box.clear();
  sc_box.clear();
  rot_boxes.clear();
...
void RotamerCalculator::transform(const IMP::atom::Hierarchy &protein,
  ...
  for (size_t i = 0; i != num_res; ++i) {
    IMP::atom::Residue rd_i(mhs[i]);
    // this got changed a bit- now set a random probability, and 
    unsigned best = 0;
    //define distrubution between 1 and length of residue entries
    double x1 = IMP::get_random_double_uniform(0,rotamers[i].get_size());
    double xcoin = IMP::get_random_double_uniform(-1,1);
    //std::cout << x0 << std::ends;
    ...
    while (best_prob < 1) {
      if (xcoin > 0) {
	      ...
	      j = IMP::get_random_double_uniform(1,rotamers[i].get_size());
	}
    }
    if (best > 0) {
